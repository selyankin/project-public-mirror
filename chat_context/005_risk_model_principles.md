# Flaffy — Risk Model Principles

## 1. Назначение документа

Этот документ фиксирует принципы того, **как Flaffy считает и объясняет риск**.
...
> Flaffy оценивает риск как **вероятностный сигнал на основании фактов**, а не как утверждение о нарушениях.
...
Flaffy использует концепт **сигналов** — правил, которые преобразуют факты в вклад в риск.
...
Flaffy использует нормализованную агрегацию:
...
Этот файл — «конституция» того, как Flaffy считает риск.
