"""Тесты получения HTML карточки дела kad.arbitr.ru."""
...
async def test_get_case_card_html_warmup_once() -> None:
    ...
    html_first = await client.get_case_card_html(case_id='123')
    html_second = await client.get_case_card_html(case_id='123')
...
async def test_get_case_card_html_403_raises_blocked() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path == '/':
            return httpx.Response(200, text='ok')
